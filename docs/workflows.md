...
**Reaching an end event can mean something to the host.** An end event carries `hostTag` like any
other flow node, so the way a process finishes is what places the host's last state.
