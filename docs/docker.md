...
same network, to `IAP_DOCLING_CALLBACK_URL`, authenticated with the shared
`IAP_DOCLING_CALLBACK_JWT` both containers read from `.env`. Neither side does asynchronous parsing
without it.
