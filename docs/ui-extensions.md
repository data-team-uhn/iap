...
Current entries: Branding (start), and the dark mode toggle, notifications bell, administration
console button (admins only, see [the admin console](#the-admin-console)), and user menu (end).
The middle section is reserved for e.g. a future search bar. A high-visibility element
...
the available administrative tools, reached through a gear button in the app bar. Each tool is a
...
| Point id | Node name | Purpose |
| --- | --- | --- |
| `iap/adminDashboard/entry` | `AdminDashboard` | Administrative tool widgets. `iap:extensionName` and `iap:subtitle` title the widget frame, `iap:extensionRenderURL` names the summary component, and `iap:targetURL` is the tool's page — typically exposed as a header action next to the title via `iap:actionLabel: "Configure"`. All the [dashboard frame properties](#the-dashboard) apply. |
...
heading and an optional main action slot. The chrome is deliberately borderless and
breadcrumb-free — wayfinding is left to the shell (e.g. a future breadcrumb extension on
`pageTop`). The first tool is the category manager (`/admin/categories`, from the `categories`
module).
...
non-administrators the gear button, the `/admin` route, and every tool simply don't exist. The
