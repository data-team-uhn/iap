...
condition hold?" for a given **context** — the piece of content the question is about, e.g. a
submission when deciding which of its schema's requirements apply. Both sides are Sling Models,
never raw resources: a `Condition` and the `Content` it is evaluated against.

```java
@Reference
private ConditionEvaluator evaluator;
...
// submission is a Content model, e.g. Submission
boolean satisfied = this.evaluator.isSatisfied(condition, submission);
boolean applies = this.evaluator.applies(requirement, submission);
```
...
- **`answer`** — the recorded answer to a question. The operand's `value` identifies the question
  ...
  **nearest-scope-first**: the context's own subtree is checked before widening,
...
passes it the operand definition and the context, both as models. A resolver returns its values in
their natural stored types — coercion is the evaluator's job — declaring the type it is
authoritative about, if any, via `Operand.of(value, type)`. It returns an empty operand (never
`null`) when nothing matches, so `is empty` can test for absence.
...
The `Content` model is all a resolver needs to walk the repository: `get(name)` and
`get(name, type)` read properties, `getParent()`, `getChild(name, type)` and `getChildren(type)`
navigate, `getReference(identifier, type)` follows a JCR reference, and `isOfType(resourceType)`
tests the type. `OperandResolver.findEnclosingEntity(content)` is the shared helper for the common
"which record is this about?" walk — the built-in `tags` and `property` resolvers both use it.
