...
describe("SubmissionView", () => {
  ...
  // The send step waiting, a form insisting on the proposal document, and the submission as given
  function servingWithSendStepAsking(submission: unknown, formExtras: Record<string, unknown> = {}) {
    return vi.fn((url: string) => {
      ...
      if (url.endsWith(".form.json")) {
        return jsonResponse({ ...EMPTY_FORM, requirements: [ PROPOSAL_REQUIREMENT ], ...formExtras });
      }
  ...
  describe("the documents section", () => {
    ...
    it("says why nothing was filled in when the reading stopped", async () => {
      const message = "Not able to safely identify whether the submitted document is a proposal";
      vi.stubGlobal("fetch", serving(projection([], { extraction: { status: "undetermined", message } })));
