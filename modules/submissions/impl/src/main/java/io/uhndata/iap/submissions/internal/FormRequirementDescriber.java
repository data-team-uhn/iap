...
/**
 * The questions a form requirement asks, in the order the schema puts them, with the answers the submission already
 * holds and with everything that does not currently apply left out.
 *
 * <p><strong>Why conditions are resolved here rather than in the browser.</strong> {@link ConditionEvaluator} is
 * extensible through a whiteboard of operand resolvers, which a downstream project may add to; an editor that
 * evaluated conditions itself could not see those, could not know it could not see them, and would silently hide
 * content because a condition it cannot evaluate is never satisfied. So the browser is told <em>what to show</em>
 * rather than what to work out, and the same evaluator that decides whether a submission is complete decides what
 * its form looks like — the two can never disagree.</p>
 *
...
public class FormRequirementDescriber implements RequirementDescriber
{
    ...
    @Override
    public void describe(final Requirement requirement, final Submission submission,
        final JsonObjectBuilder json)
    {
        // The submission's own index, which is also what decides whether its form requirements are fulfilled: two
        // indexes that disagreed about what counts as an answer would have the form and the decision to accept it
        // disagree too. Read here rather than handed in, so that the extension point does not carry a derived
        // index that only this one kind of requirement wants
        final Map<String, List<String>> answers = submission.getAnswersByQuestion();
        json.add(ITEMS,
            items(((FormRequirement) requirement).getChildren(), requirement.getName(), submission, answers));
    }
