...
public class IngestParseHandler implements ServiceTaskHandler
{
    ...
    public void execute(final WorkflowTaskContext context) throws WorkflowException, PersistenceException
    {
        ...
        if (Boolean.TRUE.equals(event.get(ParseCompletionHandler.SUCCEEDED))) {
            ...
            final String reported = string(event, ParseCompletionHandler.ERROR);
            properties.put(ParsePropertyNames.PARSE_ERROR,
                reported == null ? "The parse failed without details" : reported);
        }
    }
    ...
    /**
     * How large the daemon measured the document to be, when it said.
     *
     * @param event the parse outcome
     * @return the token count, or {@code null} when the event carries none
     */
    private static Long tokens(final WorkflowEvent event)
    {
        final Object value = event.get(ParseCompletionHandler.TOKENS);
