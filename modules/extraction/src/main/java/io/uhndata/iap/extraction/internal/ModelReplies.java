...
final class ModelReplies
{
    ...
    /**
     * The strings in an array, skipping anything in it that is not one.
     *
     * @param object the object holding the array
     * @param key its key
     * @return the strings, empty when the key is absent or not an array
     */
    static List<String> readStrings(final JsonObject object, final String key)
    {
        ...
        for (final JsonValue value : object.getJsonArray(key)) {
            if (value.getValueType() == JsonValue.ValueType.STRING) {
                strings.add(((JsonString) value).getString());
            }
        }
        return strings;
    }
