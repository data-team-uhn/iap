...
describe("toPropertyFilters", () => {
  ...
  it("maps number operators to range comparators", () => {
    expect(filters(
      ...
      { field: "amount", operator: "<", value: "9" },
      { field: "amount", operator: "<=", value: "8" },
    )).toEqual([
      { name: "amount", comparator: "=", value: "1" },
      { name: "amount", comparator: "<>", value: "3" },
      { name: "amount", comparator: ">", value: "2" },
      { name: "amount", comparator: ">=", value: "5" },
      { name: "amount", comparator: "<", value: "9" },
      { name: "amount", comparator: "<=", value: "8" },
    ]);
  });
  ...
  it("skips conditions that cannot be applied server-side", () => {
    expect(filters(
      ...
      // Operators with no server-side translation, on text and date columns alike
      { field: "title", operator: "someFutureOperator", value: "x" },
      { field: "created", operator: "someFutureOperator", value: "2026-07-01" },
      // A malformed multi-select value (isAnyOf expects an array)
      { field: "status", operator: "isAnyOf", value: "draft" },
    )).toEqual([]);
  });
});
...
describe("withServerFilterOperators", () => {
  ...
  it("filters column types without their own mapping as text", () => {
    const [flag] = withServerFilterOperators([{ field: "flag", headerName: "Flag", type: "boolean" }]);
    expect(flag.filterOperators?.map(operator => operator.value)).toEqual(
      ["contains", "doesNotContain", "equals", "doesNotEqual", "startsWith", "endsWith",
        "isEmpty", "isNotEmpty", "isAnyOf"]);
  });
});
