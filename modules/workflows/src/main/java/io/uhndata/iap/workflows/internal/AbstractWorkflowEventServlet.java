 ...
 *
 * <p>Concrete servlets only differ in the resource types they bind, which is what brings a type under workflow
 * control.</p>
 *
 * @version $Id$
 * @since 0.1.0
 */
abstract class AbstractWorkflowEventServlet extends SlingJakartaAllMethodsServlet
{
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractWorkflowEventServlet.class);

    /**
     * The engine receiving the translated events.
     *
     * @return the injected engine
     */
    protected abstract WorkflowEngine getEngine();
        ...
        final SlingJakartaHttpServletResponse response) throws IOException
    {
        try {
            ...
            final WorkflowResult result =
                getEngine().receiveEvent(request.getResource(), new WorkflowEvent(name, payload(request)));
