 ...
 * calling an external service, sending a notification. The activity node names the handler in its
 * {@code handler} property, and the engine dispatches to the registered component with that
 * {@link #getName() name}. The activity's other properties are the handler's configuration.
 *
 * <p>This is how a project plugs its own behaviour into workflows without touching the platform. Register a
 * component with a new name, and any workflow definition may name it.</p>
 *
 * <p>Handlers work inside a transaction they do not own. Writes go through the context's resource resolver and
 * the engine commits them, once, for the whole run to quiescence. A handler that committed on its own would
 * break the promise that an event either fully happened or did not happen.</p>
 *
 * @version $Id$
 * @since 0.1.0
 */
