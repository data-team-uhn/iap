...
/**
 * Something that happened, in the domain's terms: a creation was requested, a task was decided, a deadline
 * passed. Nothing changes workflow state except in response to one.
 *
 * <p>An event is channel-blind. How it arrived is the business of the translator that built it. By the time the
 * {@link WorkflowEngine engine} sees it, only the {@link #getName() name} and the {@link #getPayload() payload}
 * remain, and the name is what gets matched against the message names of waiting catching events.</p>
 *
 * @version $Id$
 * @since 0.1.0
 */
