...
describe("workflows entry point", () => {
  beforeEach(() => {
    vi.resetModules();
    document.body.innerHTML = "";
  });

  it("renders the editor into its container", async () => {
    document.body.innerHTML = '<div id="main-container"></div>';

    await act(async () => {
      await import("@iap/workflows/Workflows");
    });

    expect(document.querySelector('[data-testid="bpmn-editor"]')).toBeInTheDocument();
  });
  ...
  it("does nothing on a page without the container", async () => {
    ...
    expect(document.querySelector('[data-testid="bpmn-editor"]')).not.toBeInTheDocument();
  });
});
