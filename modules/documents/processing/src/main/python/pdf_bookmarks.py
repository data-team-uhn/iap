...
def _get_pdf_bookmarks(reader) -> list[dict]:
    ...
    _flatten(reader, reader.outline, 1, pdf_bookmarks)
    return pdf_bookmarks


def extract_bookmarks(source) -> list[dict]:
    """Flatten a PDF's bookmark outline into ordered ``{title, level, page}`` bookmarks.
    ...
    try:
        if isinstance(source, (str, Path, PurePath)):
            ...
            with open(source, "rb") as handle:
                return _get_pdf_bookmarks(open_pdf_reader(handle))
        return _get_pdf_bookmarks(source)
    except Exception:  # noqa: BLE001 -- any reader/parse failure means "no usable bookmarks"
        return []


def _flatten(reader, items, level: int, out: list[dict]) -> None:
    """Walk a (nested) outline into flat bookmarks, one recursion per nesting level."""
