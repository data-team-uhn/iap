...
class TestRamBudget:
    def test_zero_total_uses_available_headroom(self):
        assert bs.calc_ram_budget_gb(0, 16) == 16 * bs.RAM_AVAILABLE_HEADROOM

    def test_tight_available_uses_available_headroom(self):
        # available < total * RAM_TIGHT_FRACTION (0.35)
        assert bs.calc_ram_budget_gb(64, 10) == 10 * bs.RAM_AVAILABLE_HEADROOM

    def test_normal_uses_min_of_both_headrooms(self):
        # 64 GB total, 40 GB available: min(64*0.85, 40*0.90) = min(54.4, 36.0) = 36.0
        expected = min(64 * bs.RAM_TOTAL_HEADROOM, 40 * bs.RAM_AVAILABLE_HEADROOM)
        assert bs.calc_ram_budget_gb(64, 40) == expected


class TestMaxWorkersByRam:
    def test_floor_division_by_gb_per_worker(self):
        assert bs.calc_max_workers_by_ram(10.0) == int(10.0 // bs.GB_PER_WORKER)
    ...
    def test_minimum_of_one(self):
        ...
        assert bs.calc_max_workers_by_ram(1.0) == 1
        assert bs.calc_max_workers_by_ram(0.0) == 1


class TestCalcWorkers:
    def test_override_wins(self):
        assert bs.calc_workers(workers_override=7) == 7

    def test_default_is_positive(self):
        assert bs.calc_workers() >= 1
        assert bs.calc_workers() == bs.DEFAULT_MAX_WORKERS

    def test_non_positive_override_clamped_to_one(self):
        # ProcessPoolExecutor(max_workers=0) raises, so 0 must never reach it.
        assert bs.calc_workers(workers_override=0) == 1
        assert bs.calc_workers(workers_override=-4) == 1


class TestBatchPages:
    def test_override_wins(self):
        assert bs.calc_batch_pages(1000, 4, batch_pages_override=9) == 9

    def test_non_positive_pages_is_one(self):
        assert bs.calc_batch_pages(0, 4) == 1
    ...
    def test_small_document_uses_one_page_batches(self):
        ...
        assert bs.calc_batch_pages(10, 4) == 1


class TestChunkCount:
    def test_zero_pages(self):
        assert bs.calc_chunk_count(0, 4) == 0

    def test_ceiling_division(self):
        assert bs.calc_chunk_count(10, 4) == math.ceil(10 / 4)  # 3


class TestActiveWorkers:
    def test_no_chunks_keeps_at_least_one_worker(self):
        assert bs.calc_active_workers(8, 0) == 8
        assert bs.calc_active_workers(0, 0) == 1

    def test_never_more_workers_than_chunks(self):
        assert bs.calc_active_workers(8, 3) == 3

    def test_never_more_chunks_than_workers(self):
        assert bs.calc_active_workers(2, 10) == 2


class TestBatchPagesOverrideClamped:
    def test_non_positive_override_clamped_to_one(self):
        # This value becomes a range() step; a step of 0 raises.
        assert bs.calc_batch_pages(100, 4, batch_pages_override=0) == 1
        assert bs.calc_batch_pages(100, 4, batch_pages_override=-2) == 1

    def test_clamped_override_still_produces_usable_page_ranges(self):
        batch = bs.calc_batch_pages(10, 4, batch_pages_override=0)
