...
:mod:`daemon_utils` alone, so it runs everywhere.
"""
...
class TestDaemonToken:
    def test_unset_is_none(self, monkeypatch):
        monkeypatch.delenv(daemon_utils.TOKEN_ENVIRONMENT_VARIABLE, raising=False)
        assert daemon_utils.get_daemon_token() is None

    def test_blank_is_none(self, monkeypatch):
        # The compose file leaves it empty by default, which must mean "unset", not "".
        monkeypatch.setenv(daemon_utils.TOKEN_ENVIRONMENT_VARIABLE, "   ")
        assert daemon_utils.get_daemon_token() is None

    def test_surrounding_whitespace_is_stripped(self, monkeypatch):
        monkeypatch.setenv(daemon_utils.TOKEN_ENVIRONMENT_VARIABLE, "  s3cret\n")
        assert daemon_utils.get_daemon_token() == "s3cret"
...
class TestRefuseUnauthorized:
    ...
    def test_no_token_configured_lets_a_plain_caller_through(self, monkeypatch):
        monkeypatch.delenv(daemon_utils.TOKEN_ENVIRONMENT_VARIABLE, raising=False)
        handler = FakeHandler()
        assert daemon_utils.refuse_unauthorized(handler, "/parse", log=_log) is False
        assert handler.sent == []
    ...
    def test_a_browser_origin_is_refused_even_with_no_token(self, monkeypatch):
        ...
        assert daemon_utils.refuse_unauthorized(handler, "/parse", log=_log) is True
    ...
    def test_anything_but_the_right_token_is_401(self, monkeypatch, header):
        ...
        assert daemon_utils.TOKEN_ENVIRONMENT_VARIABLE.encode() in handler.body()
    ...
    def test_the_refusal_is_logged_with_the_endpoint(self, monkeypatch):
        ...
        lines: list[str] = []
        handler = FakeHandler(headers={"Origin": "https://evil.example"})
        daemon_utils.refuse_unauthorized(handler, "/shutdown", log=lines.append)
...
class TestDrainRequestBody:
    ...
    def test_the_whole_body_is_read(self):
        ...
        daemon_utils.drain_request_body(handler)
    ...
    def test_an_oversized_declared_length_is_refused_rather_than_read(self):
        ...
        handler = FakeHandler(b"abc")
        handler.set_header("Content-Length", str(daemon_utils.MAX_DRAINED_BODY_BYTES + 1))
        daemon_utils.drain_request_body(handler)
        assert handler.close_connection is True
    ...
    def test_a_length_at_the_cap_is_still_drained(self):
        ...
        handler.set_header("Content-Length", str(daemon_utils.MAX_DRAINED_BODY_BYTES))
        daemon_utils.drain_request_body(handler)
...
class TestJsonResponse:
    def test_it_writes_json_with_a_length_and_content_type(self):
        handler = FakeHandler()
        daemon_utils.send_json_response(handler, HTTPStatus.OK, {"status": "ok"})
    ...
    def test_non_ascii_survives_the_reply(self):
        handler = FakeHandler()
        daemon_utils.send_json_response(handler, HTTPStatus.BAD_REQUEST, {"error": "café — 研究"})
        assert json.loads(handler.body())["error"] == "café — 研究"

    def test_a_closing_connection_says_so(self):
        handler = FakeHandler()
        handler.close_connection = True
        daemon_utils.send_json_response(handler, HTTPStatus.OK, {})
...
class TestDrainReportsRefusal:
    ...
    def test_it_returns_true_when_the_body_is_dealt_with(self):
        assert daemon_utils.drain_request_body(FakeHandler(b"x" * 100)) is True
