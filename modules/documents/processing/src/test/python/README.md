...
| `test_chunker_internals.py` | splitting internals via `build_chunk_tree`: `normalize_title`, block packing, oversized splitting, paragraph fallback, heading resolution, running-header suppression, merge passes that avoid bare-heading or tiny orphan chunks |
...
`chunker.write_chunk_files` is what puts the outline (and markdown / chunks) on disk. Tests
