...
Lists every extension point defined in the codebase by finding the `ext:Point` node
...
`ext:pointId` and human-readable name.
...
Scaffolds a new extension point: it writes the `ext:Point` node definition into the
...
Imports a content JSON file — typically an `ext:Extension` — straight into a running instance via
...
> `defaultOrder`) overwrite the node type's autocreated defaults instead of silently keeping
